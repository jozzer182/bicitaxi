...
struct RideRequest: Identifiable {
    ...
    // MARK: - Compatibility Properties for DriverMapView
    
    /// CLLocationCoordinate2D from pickup location (for map annotations)
    var coordinate: CLLocationCoordinate2D {
        CLLocationCoordinate2D(latitude: pickup.lat, longitude: pickup.lng)
    }
    
    /// Title for display/annotations
    var title: String {
        "Solicitud de Viaje"
    }
    
    /// Estimated fare (placeholder, will be calculated)
    var estimatedFare: Int {
        // Base fare + distance-based estimate
        5000
    }
    
    // MARK: - Convenience Initializer for Dummy Data
    
    /// Creates a dummy RideRequest for map testing
    init(coordinate: CLLocationCoordinate2D, title: String, estimatedFare: Int) {
        self.id = UUID().uuidString
        self.requestId = self.id
        self.createdByUid = "dummy"
        self.pickup = LocationPoint(lat: coordinate.latitude, lng: coordinate.longitude)
        self.dropoff = nil
        self.status = .open
        self.assignedDriverUid = nil
        self.createdAt = Date()
        self.updatedAt = Date()
        self.cellId = "dummy"
        ...
        self.driverLat = nil
        self.driverLng = nil
        self.driverLocationUpdatedAt = nil
...
class RequestService: ObservableObject {
    ...
    @Published var isWatchingExpanded: Bool = false
    ...
    func assignDriver(cellId: String, requestId: String, driverUid: String) async {
        ...
        do {
            let userDoc = try await db.collection("users").document(driverUid).getDocument()
